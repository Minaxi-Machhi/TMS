<template>
  <v-row style="position: relative">
    <div
      class="d-flex align-center ga-2 px-3"
    ...
    >
        ...
        <v-list class="mt-1 border" style="max-height: 50vh; overflow-y: auto">
          ...
          >
            <template #item="{ element: col, index }">
              <v-list-item
                v-if="!['ag-Grid-SelectionColumn'].includes(col.colId)"
              ...
              >
                <v-list-item-title>
                  <div class="d-flex">
                    <span
                      class="d-flex align-center justify-space-betweeen w-100"
                    >
                      <v-checkbox
                        :false-value="false"
                        :true-value="true"
                        ...
                        @change="
                          (savedState[index].hide = !savedState[index].hide),
                            toggleColumnVisibility(
                              col.colId,
                              !savedState[index].hide
                            )
                        "
                      ></v-checkbox>
                      <span
                        class="font-weight-light ml-1 text-capitalize"
      ...
      <v-menu>
        ...
        <v-list
          class="mt-1 border"
          style="max-height: 50vh; min-height: auto; overflow-y: auto"
        >
          <template v-for="(config, key) in filterManagerConfig" :key="key">
            <v-list-item
              style="min-height: unset"
              v-if="!['limit', 'offset'].includes(key)"
              @click.stop
            >
              <v-list-item-title>
                <div class="d-flex align-center">
                  <v-checkbox
                    v-model="visibilityList"
                    :value="key"
                  ></v-checkbox>
                  <span
                    class="font-weight-light mr-2 ml-1"
                    style="font-size: small"
                    >{{ config.label }}</span
                  >
                </div>
      ...
      <v-btn
        ...
        @click="dialog = true"
        :disabled="
          visibilityList.length === Object.keys(filterManagerConfig).length
        "
      >
        <v-icon size="14" color="white"> mdi-filter-plus </v-icon>
      ...
      <v-badge dot v-model="hasActiveFilters" :color="getColor('red')">
        ...
        >
          <v-icon size="15" color="white"> mdi-filter-off </v-icon>
    ...
    <template v-for="(config, key) in filterManagerConfig" :key="key">
      <v-col v-if="visibilityList.includes(key)" :cols="columnsLength">
        <!-- v-autocomplete -->
        <v-autocomplete
          v-if="config.type === 'v-autocomplete'"
          class="text-capitalize"
          density="compact"
          v-model="filterManager[key]"
          :label="config.label"
          :items="config.items || []"
          :item-title="config.itemTitle"
        ...
        <!-- InputField -->
        <InputField
          v-else-if="config.type === 'InputField'"
          v-model="filterManager[key]"
          :label="config.label"
          @update:modelValue="debouncedApplyFilter"
          :prepend-inner-icon="config.icon || ''"
          clearable
        />
        ...
        <!-- DateRangePicker -->
        <DateRangePicker
          v-else-if="config.type === 'DateRangePicker'"
          v-model="filterManager[key]"
  ...
  >
    <v-card>
      <v-card-title class="px-6 py-2 d-flex align-center justify-space-between">
        <div class="d-flex align-center">
          <v-icon size="x-small" :color="getColor('orange')" class="mr-3"
            >mdi-filter-plus</v-icon
          >
          <span class="text-h5 text-capitalize"> More Filters </span>
        </div>
      ...
      <v-card-actions class="mr-4 mb-1">
        <v-btn color="success" variant="elevated" @click="applyFilter"
          >Apply</v-btn
        >
      </v-card-actions>
    </v-card>
...
const visibilityList = ref(
  localStorageUtility.getItemFromLocalStorage(
    "project-filter-visibility-list"
  ) || ["search"]
);
...
const hasActiveFilters = computed(() => {
  return !!filterManager.value.search;
});
...
watch(
  savedState,
  (newList) => {
    localStorageUtility.setItemToLocalStorage("project-list-state", newList);
  },
  { deep: true }
);
