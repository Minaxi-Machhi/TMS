...
const columnDefs = ref([
    ...
    },
    {
        ...
        field: "branch_name",
        sortable: true,
        headerTooltip: "Name of the branch",
        tooltipField: "branch_name",
    },
    {
        colId: "default",
        headerName: "Default",
        field: "verbose_default",
        sortable: true,
        headerTooltip: "Default project?",
        tooltipField: "verbose_default",
    },
    {
        colId: "added_on",
        headerName: "Added On",
...
const applyFilter = async () => {
    let filtersFromLocalStorage = localStorageUtility.getItemFromLocalStorage('project-list-filter-manager');
    projectFilters.value = { ...projectFilters.value, ...filtersFromLocalStorage, offset: 0 };
...
const setGridColumnVisible = (colId, shouldShow) => {
    ...
    localStorageUtility.setItemToLocalStorage('project-list-state', gridApi.value.getColumnState());
}

// method to reorder columns of the grid
const reorderColumns = () => {
    const savedState = localStorageUtility.getItemFromLocalStorage('project-list-state');
...
const handlePagination = async ({ page, itemsPerPage }) => {
    ...
    const filtersFromLocalStorage = localStorageUtility.getItemFromLocalStorage("project-list-filter-manager");
    localStorageUtility.setItemToLocalStorage("project-list-filter-manager", { ...filtersFromLocalStorage, ...projectFilters.value });
    await getProjectList(projectFilters.value);
}
...
const handleSorting = async (params) => {
    ...
    localStorageUtility.setItemToLocalStorage('project-list-state', params.api.getColumnState());

    let filtersFromLocalStorage = localStorageUtility.getItemFromLocalStorage('project-list-filter-manager');
    projectFilters.value = { ...filtersFromLocalStorage, ...projectFilters.value, offset: 0 };
    localStorageUtility.setItemToLocalStorage('project-list-filter-manager', projectFilters.value);
    await getProjectList(projectFilters.value);
}
...
onMounted(async () => {
    ...
    if (filtersFromLocalStorage) {
        projectFilters.value = { ...projectFilters.value, ...filtersFromLocalStorage };
    } else {
        localStorageUtility.setItemToLocalStorage('project-list-filter-manager', projectFilters.value);
    }
});
...
<template>
    ...
    <v-row>
        <v-col class="d-flex align-center justify-end ga-3" style="margin-left: 9.375rem;">
            ...
            <v-btn class="text-uppercase d-flex align-center" density="compact" variant="tonal"
                @click="currentProjectId = null; addEditProjectDialogVisible = true">
    ...
    <v-card class="mt-4">
        ...
        <v-card-text>
            <ag-grid-vue style="width: 100%; height: 55vh;" :rowHeight="40" :rowData="projectList"
                :columnDefs="columnDefs" :defaultColDef="defaultColDef" :gridOptions="gridOptions"
                :tooltipShowDelay="500" :tooltipMouseTrack="true" :autoSizeStrategy="{ type: 'fitGridWidth' }"
                :rowSelection="{ mode: 'singleRow' }" :selectionColumnDef="{ pinned: true }"
                @gridReady="handleGridReady" @sortChanged="handleSorting" @rowClicked="handleRowClicked"
        ...
        <v-card-actions class="mb-3">
            ...
            variant="outlined" @input="handleSearch"></v-text-field> -->
                    ...
                    <v-btn @click="onExport" color="grey" class="text-uppercase" density="compact" variant="tonal"
                        size="small" prepend-icon="ri-upload-2-line">
                        Export
                    </v-btn>
                </v-col>

                <v-col cols="6">
                    <Pagination :total-items="totalRecords" :items-per-page="itemsPerPage"
    ...
    <!-- dialogs -->
    <AddEditProjectDialog v-if="addEditProjectDialogVisible" :dialogVisible="addEditProjectDialogVisible"
        :id="currentProjectId" @close="closeAddEditProjectDialog">
