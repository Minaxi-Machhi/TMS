...
const reorderColumns = () => {
  const savedState =
    localStorageUtility.getItemFromLocalStorage("user-list-state");
...
const handlePagination = async ({ page, itemsPerPage }) => {
  ...
  localStorageUtility.setItemToLocalStorage("user-list-filter-manager", {
    ...filtersFromLocalStorage,
    ...userFilters.value,
  });
  await getUserProfileList(userFilters.value);
...
<template>
  ...
  >
    <AddEditUserDrawer
      ...
      @close="
        async () => {
          (drawer = false), await getUserProfileList(userFilters.value);
        }
      "
    >
    </AddEditUserDrawer>
  ...
  <v-row>
    ...
    >
      ...
      <v-btn
        ...
        "
      >
        <v-icon size="12"> ri-add-fill </v-icon>
        <v-icon size="14"> ri-user-fill </v-icon>
